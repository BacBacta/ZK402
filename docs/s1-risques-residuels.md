# S1 — Mesures et risques résiduels (état au 25 septembre 2026, incréments 1 à 5 bis)
...
| R7 | ~~Latence qui croît avec la taille du lot~~ **Traité (P3, solution 1)** : séquestre au plafond à la soumission, phase Fills sans multiplication, délai de grâce on-chain avant Apply | La latence croît encore, mais lentement ; le débit global du coprocesseur (≈ 1 mul 64 bits/s) reste hors de notre contrôle | Mesuré : **64 ordres → dernière exécution lisible en 22,4 s** (médiane 21,0 s) ; 32 ordres → 14,6 s. Objectif « 64 en < 60 s » **atteint** | Débit Fhenix (mainnet, GPU) ; 16 bits pour augmenter le débit global |
| R22 | **Coûts de la solution 1** : l'acheteur immobilise 3 % de plus ; un mouvement > 3 % pendant le lot l'annule ; la 1ʳᵉ soumission exige des oracles frais ; soldes définitifs ≈ 40 s après les exécutions | Conséquence directe du séquestre au plafond et du délai de grâce | Aucune perte possible : le lot annulé rembourse tout ; les sorties attendent la fin d'Apply | Bande `CAP_BAND_BPS` calibrée sur la volatilité réelle ; délai calibré sur le débit mainnet |
